...
async function configure() {
  console.log(chalk.cyan("\u{1F527} Configura\xE7\xE3o do bimmo-cli\n"));
  ...
  if (action === "language") {
    const { lang } = await inquirer.prompt([{
      ...
      choices: [
        { name: "Portugu\xEAs (Brasil)", value: "pt-BR" },
  ...
  const answers = await inquirer.prompt([
    {
      type: "input",
      name: "profileName",
      message: "D\xEA um nome para este perfil:",
      validate: (i) => i.length > 0 || "Nome obrigat\xF3rio"
    },
    {
      type: "list",
      name: "provider",
    ...
    },
    {
      ...
      name: "apiKey",
      message: "API Key:",
      validate: (i) => i.length > 5 || "Chave inv\xE1lida"
    },
    {
      type: "input",
      name: "model",
      message: "Modelo padr\xE3o (vazio para default):"
    },
    {
      type: "input",
      name: "customBaseURL",
...
async function configureAgents() {
  ...
  if (action === "Criar Agente") {
    ...
    const answers = await inquirer.prompt([
      ...
      { type: "list", name: "profile", message: "Qual perfil este agente usar\xE1?", choices: profileList },
      { type: "input", name: "modelOverride", message: "Sobrescrever modelo (vazio para manter o do perfil):" },
      { type: "list", name: "mode", message: "Modo padr\xE3o:", choices: ["chat", "plan", "edit"] },
